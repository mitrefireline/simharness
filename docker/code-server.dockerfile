...
COPY poetry.lock pyproject.toml README.md LICENSE simharness2/ .
...
RUN curl -sSkL https://install.python-poetry.org | python - &&\
    ...
    poetry install --only codeserver

# Deployment Stage
FROM linuxserver/code-server as deploy
...
RUN sudo ln -s /venv/bin/python /usr/local/bin/python

# Copy the needed code
WORKDIR /code
COPY README.md LICENSE simharness2 configs/ .
