...
class ReactiveHarness(RLHarness):  # noqa: D205,D212,D415
    ...
    def step(
        ...
        if sim_run and self.harness_analytics:
            self.harness_analytics.update_sim_after_one_simulation_step(
                timestep=self.timesteps
            )
    ...
    def get_nonsim_attribute_bounds(self) -> OrderedDict[str, Dict[str, int]]:  # noqa
        ...
        nonsim_min_maxes["bench_fire_map"] = {"min": 0, "max": self.sim_agent_id}
        nonsim_min_maxes["bench_fire_map_final"] = {"min": 0, "max": self.sim_agent_id}
