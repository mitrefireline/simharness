"""
Sample reactive.py env for simharness 2.0 with integrated Reward Class that inherits FEAR data extraction and dual (Benchmark + Agent) simulation capabilites
    - Built from afennelly current version of simharness 2.0 environment

* New additions are commented with #NEW:
#--------------------------------------------------------------------------------------------
#NEW:
##(dgandikota)
#--------------------------------------------------------------------------------------------
...
class ReactiveHarness(RLHarness):  # noqa: D205,D212,D415
    ...
    def __init__(
        ...
        #--------------------------------------------------------------------------------------------
        #NEW: TODO create variable that tracks the number of timesteps that have occurred within an episode
        ##(dgandikota)
        self.timestep = 1
        #--------------------------------------------------------------------------------------------
    ...
    def step(
        ...
        #--------------------------------------------------------------------------------------------
        #NEW: Update the FEAR Data after each agents step/action
        ##(dgandikota)
        ...
        if self.num_agent_steps % self.agent_speed == 0:
            ...
            #update agent Sim based off the agents actions
            ...
            #--------------------------------------------------------------------------------------------
            #NEW: Update the benchmark sim that has no agent actions within
            ##(dgandikota)
            bench_sim_fire_map, bench_sim_active = self.bench_simulation.run(1)
            bench_fire_map = np.copy(bench_sim_fire_map)
            #bench_fire_map[self.agent_pos[0]][self.agent_pos[1]] = self.sim_agent_id

            #--------------------------------------------------------------------------------------------

            #--------------------------------------------------------------------------------------------
            #NEW: Update the Reward Class - FEAR Data for each new simulation timestep of the benchmark simulation with no mitigations
            ##(dgandikota)
            ...
            #--------------------------------------------------------------------------------------------
            #NEW: Update the Reward Class - FEAR Data for each new simulation timestep of the simulation with the Agent
            ##(dgandikota)
            self.env_Reward.timestep_AgentSim_FEAR_Update(self.timestep, fire_map, sim_active)
            #--------------------------------------------------------------------------------------------
            
            #--------------------------------------------------------------------------------------------
            #NEW: Calculate the reward using the FEAR Data class TODO-Make parent class that inherits and defines reward
            ##(dgandikota)
            ...
            #NEW: FIXME increment the timestep of the episode
    ...
    def reset(
        ...
        *,
        ...
        if not self.deterministic:
            ...
            #--------------------------------------------------------------------------------------------
            #NEW: set seeds of benchmark simulation
            ##(dgandikota)
        ...
        #--------------------------------------------------------------------------------------------
        #NEW: reset benchmark simulation
        ##(dgandikota)
        ...
        #TODO in future find way to add benchmark sim to agents state
        ...
        #--------------------------------------------------------------------------------------------
        #NEW: update the benchmark simulation - Not sure if actually needed but can't hurt
        ##(dgandikota)
    ...
    def render(self):  # noqa
        ...
        #TODO if wanted - add rendering capabilities to the Benchmark Simulation
    ...
    def _calculate_reward_manual(self, fire_map: np.ndarray) -> float:
