...
class ComplexObsReactiveHarness(ReactiveHarness[AnyFireSimulation]):
    def __init__(self, **kwargs):
        ...
        # if self.attributes != [FIRE_MAP_KEY, AGENT_POSITION_KEY]:
        #     raise AssertionError(
        #         f"The `ComplexObsReactiveHarness` requires `self.attributes` to be "
        #         f"[{FIRE_MAP_KEY}, {AGENT_POSITION_KEY}]."
        #     )
    ...
    def _get_fire_map_observation_space(self) -> spaces.Box:
        """TODO."""
        # Ensure AGENT_POSITION_KEY is not in `self.sim_attributes`.
        """
        if AGENT_POSITION_KEY in self.nonsim_attributes:
            self.nonsim_attributes.pop(self.nonsim_attributes.index(AGENT_POSITION_KEY))
        """

        fire_map_attributes = self.attributes.copy()
        fire_map_attributes.pop(fire_map_attributes.index(AGENT_POSITION_KEY))
        ...
        # TODO: Note that the following attributes DO NOT reflect AGENT_POSITION_KEY.
