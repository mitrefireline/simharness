...
class MARLReactiveHarness(RLHarness):  # noqa: D205,D212,D415
    ...
    def __init__(
        ...
        self.spec = EnvSpec(
            id="MARLReactiveHarness",
            entry_point="simharness2.environments.reactive:MARLReactiveHarness",
    ...
    def step(
        ...
        for id_num in range(self.num_agents):
            ...
            points.append([agent_pos[1], agent_pos[0], self.sim_agent_id + id_num])
        self.simulation.update_agent_positions(points)
        ...
        for id_num in range(self.num_agents):
            agent_pos = self.agent_pos[id_num]
            fire_map[agent_pos[0]][agent_pos[1]] = self.sim_agent_id + id_num
        ...
        for id_num in range(self.num_agents):
            ...
            if not sim_active:
                terminateds[agent_id] = not sim_active
            infos[agent_id] = {}
            
        terminateds["__all__"] = len(terminateds) == self.num_agents
        truncateds["__all__"] = len(truncateds) == self.num_agents
    ...
    def get_nonsim_attribute_bounds(self) -> OrderedDict[str, Dict[str, int]]:  # noqa
        ...
        nonsim_min_maxes["fire_map"] = {"min": 0, "max": self.sim_agent_id + self.num_agents - 1}
    ...
    def get_nonsim_attribute_data(self) -> OrderedDict[str, np.ndarray]:  # noqa
        ...
        # Place the agent on the fire map using the agent ID.
            nonsim_data["fire_map"][agent_pos[0]][agent_pos[1]] = self.sim_agent_id + id_num
    ...
    def _set_agent_pos_for_episode_start(self):
        ...
        for i in range(self.num_agents):
            if self.randomize_initial_agent_pos[i]:
                self.agent_pos[i] = self.np_random.integers(
                ...
                self.agent_pos[i] = self.initial_agent_pos[i]



class MARLReactiveDiscreteHarness(MARLReactiveHarness):  # noqa: D205,D212,D415
    """
    ### Description
    ...
    def step(
        ...
        for id_num in range(self.num_agents):
            agent_id = f"agent_{id_num}"
            movement = actions[agent_id] % len(self.movements)
            interaction = int(actions[agent_id] / len(self.movements))
