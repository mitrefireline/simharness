...
class RenderEnvMARL(DefaultCallbacks):
    ...
    def on_episode_end(
        ...
        **kwargs,
        ...
        env: MARLReactiveHarness = base_env.envs[vector_idx]
        ...
        if not headless and in_evaluation:
            ...
            if hasattr(base_env.envs[vector_idx].sim, "_game"):
                base_env.envs[vector_idx].sim.save_gif(save_path)
            else:
            ...
            sim_data = base_env.envs[vector_idx].sim.config.yaml_data
            # attempting to avoid memory leaks, etc.
            del base_env.envs[env_index].sim
            # Disable pygame and deactivate rendering mode
            sim_data["simulation"]["headless"] = True
            base_env.envs[vector_idx].sim = FireSimulation(Config(config_dict=sim_data))
