...
class InitializeSimfire(DefaultCallbacks):
    ...
    def __init__(self, legacy_callbacks_dict: Dict[str, callable] = None):
        ...
        # NOTE: We need separate maps for train and eval, incase they use diff op locs.
        self.train_loc_to_idx, self.eval_loc_to_idx = {}, {}
    ...
    def on_algorithm_init(
        ...
        **kwargs,
        ...
        sim_init_cfg = algorithm.config.env_config.get("sim_init_cfg")
        ...
        # FIXME: Review the logic here, not sure if we are raising errors as expected.
        ...
        train_locs_duplicated = []
        for loc in train_locs:
            train_locs_duplicated.extend([loc] * self.num_train_fire_init_pos)

        eval_locs_duplicated = []
        ...
        train_locs_used = algorithm.workers.foreach_worker(
            lambda w: w.foreach_env(
                lambda env: env._set_operational_location(
                    locations=train_locs_duplicated,
        ...
        eval_locs_used = algorithm.evaluation_workers.foreach_worker(
            lambda w: w.foreach_env(
                lambda env: env._set_operational_location(
                    locations=eval_locs_duplicated,
        ...
        op_loc_and_pos_used = algorithm.workers.foreach_worker(
            lambda w: w.foreach_env(
                lambda env: env._initialize_simfire(
                    data=train_subset,
                    num_envs_per_worker=self._train_envs_per_worker,
                    loc_to_idx=self.train_loc_to_idx,
                )
            ),
            local_worker=self.has_local_train_worker,
        )
        ...
        for loc_uid, pos in chain(*op_loc_and_pos_used):
        ...
        algorithm.evaluation_workers.foreach_worker(
            lambda w: w.foreach_env(
                lambda env: env._initialize_simfire(
                    data=eval_subset,
                    num_envs_per_worker=self._eval_envs_per_worker,
                    loc_to_idx=self.eval_loc_to_idx,
                )
            ),
            local_worker=self.has_local_eval_worker,
        )
    ...
    def on_train_result(
        ...
        **kwargs,
        ...
        if curr_iter % self.resample_interval == 0:
            ...
            train_data: np.ndarray = ray.get(self.data_object_refs["train"])
