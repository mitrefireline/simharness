...
def filter_fire_initial_position_data(
    ...
    **kwargs,
    ...
    """Filter the fire initial position data, `fire_df`, using the provided `query`.

    Arguments:
        fire_df: A modin DataFrame containing the fire initial position data.
    """
    ...
    # Save the filtered train/eval data to disk.
    if write_to_disk:
        if logdir is None:
            raise ValueError("`logdir` is required when `save_filtered_data` is True!")
        else:
            train_data_path = os.path.join(logdir, "train_data.npy")
            eval_data_path = os.path.join(logdir, "eval_data.npy")
            logger.info(f"Saving train data to: {train_data_path}")
            logger.info(f"Saving eval data to: {eval_data_path}")
            np.save(train_data_path, train_arr)
            np.save(eval_data_path, eval_arr)
...
def generate_fire_initial_position_data(
    ...
    if os.path.exists(data_save_path):
        ...
        pos_df: pd.DataFrame = pd.read_csv(data_save_path, dtype=FIRE_DATA_DTYPES)
        ...
        # No remaining calculations needed, just return the data
            ...
            return pos_df.sample(n=output_size, replace=False)
